...
A pairwise list is an efficient option on a CPU as it avoids nested for loops and redundant computations (each particle pair is visited only once). Bond forces can be computed in parallel (this is embarrassingly parallel) but the reduction of bond forces to particle forces must be done in serial, as multiple threads may attempt to update the force on the same particle concurrently. See `compute_nodal_forces()` in [this file](https://github.com/mark-hobbs/pypd/blob/main/pypd/kernels/particles.py) for details. 

However, a per-particle neighbour list can provide better GPU performance. This approach trades some redundant computations (each particle pair will be visited twice) for massively parallel and conflict-free memory writes, which is often a worthwhile tradeoff on a GPU.
