   ...
   ```python
    for particle in particles:
        for neighbour in particle.neighbours:
            particle.f += bonds.f[particle, neighbour] 
    ```
...
A pairwise list is an efficient option on a CPU as it avoids nested for loops and redundant computations (each particle pair is visited only once). 

because memory access patterns are predictable and force accumulation can be done on-the-fly—it introduces challenges on a **GPU** due to the need for **force reduction**.
...
* **Pairwise bond list** is efficient on **CPUs**, where force accumulation is straightforward.
* On **GPUs**, the same structure introduces serial bottlenecks due to **atomic reductions** on shared memory locations.
